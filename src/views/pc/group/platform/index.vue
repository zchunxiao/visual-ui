                ...
                <div class="platform-head platform-card-title">
                  ...
                  <div class="platform-num">
                    <span style="margin-right:15px"
                      >开工人数: <i class="orange">{{ ws.workerNum }}</i></span
                    >
                    <span
                      >开工机台数:<i class="orange">{{
                        ws.workStationNum
                      }}</i></span
                    >
                  </div>
            ...
            <dv-border-box-7>
              <div class="dv-box">
                <div class="platform-head platform-card-title">
                  ...
                  <div class="platform-num">
                    <span style="margin-right:15px"
                      >开工人数: <i class="orange">{{ ms.workerNum }}</i></span
                    >
                    <span
                      >开工机台数:<i class="orange">{{
                        ms.workStationNum
            ...
            <dv-border-box-7>
              <div class="dv-box">
                <div class="platform-head platform-card-title">
                  ...
                  <div class="platform-num">
                    <span style="margin-right:15px"
                      >开工人数: <i class="orange">{{ bp.workerNum }}</i></span
                    >
                    <span
                      >开工机台数:<i class="orange">{{
                        bp.workStationNum
...
import scrollBoardWarn from "./components/scrollBoardWarn";
import api from "@/api";
  ...
  methods: {
    createData() {
      api.getSupplyChain().then((res) => {
        ...
        let drawNeedsList = [];
        (drawNeeds || []).map((item) => {
          drawNeedsList.push([item.base, item.model, item.num, item.planTime]);
        });
        ...
        (safetyStock || []).map((item) => {
          safetyStockList.push([item.model, item.safetyStock]);
        });
        ...
        (ws.prodData || []).map((item) => {
          wsList.push([item.model, item.yield, item.stock]);
        });
        this.wsConfig = {
          header: ["型号", "产量", "库存"],
          data: wsList,
        ...
        (ms.prodData || []).map((item) => {
          msList.push([item.model, item.yield, item.stock]);
        });
        this.msConfig = {
          header: ["型号", "产量", "库存"],
          data: wsList,
        ...
        (bp.prodData || []).map((item) => {
          bpList.push([item.model, item.yield, item.stock]);
        });
        this.bpConfig = {
          header: ["型号", "产量", "库存"],
          data: wsList,
